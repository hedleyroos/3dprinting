// ============================================================
// Champagne Bottle Cradle — Wave Cushion, Lightweight Frame
//
// A smooth wave-shaped cushion that holds 2 champagne bottles lying
// on their sides in rounded troughs.  Two parallel rails (front =
...
// (270 x 270 mm bed, 256 mm Z).  2 x 100 mm bottles easily fit the
// bed width (~192 mm body) with generous margin.
//
// Coordinate system:
//   X = across the bottles (2 troughs side by side, the chain axis)
...
/* [Bottles] */
bottle_dia      = 100;   // Nominal bottle diameter (Champagne / Magnum)
bottle_count    = 2;     // Troughs per cushion
bottle_pitch    = 96;    // Centre-to-centre spacing across X
...
rail_back_w     = 30;    // Width of the back  (-Y) support rail along Y
...
cradle_r  = bottle_dia / 2 + cradle_extra_r;   // 51.5
body_w    = bottle_count * bottle_pitch;        // 192
half_w    = body_w / 2;                         // 96
top_z     = base_wall + trough_depth;           // 24
z_c       = base_wall + cradle_r;               // 57.5
half_rail_front = rail_front_w / 2;                // 7.5
half_rail_back  = rail_back_w  / 2;                // 15
...
module champagne_bottle_cushion() {
    difference() {
        union() {
            wave_body();
            end_bar(-1);  // -X end
